...
void initgrid(double **old, int lb, int ub) {
  ...
  int llb = (lb == 0) ? 1 : lb;
  int lub = (ub == nx) ? nx - 1 : ub;

  for (int a = llb, b = 0; a < lub; a++) /* boundary nodes */
    old[a][b] = randa(xu + a * dx, 0);

  for (int a = llb, b = ny - 1; a < lub; a++)
    old[a][b] = randb(xu + a * dx, 0);

  if (lb == 0) {
    for (int a = 0, b = 0; b < ny; b++)
      old[a][b] = randc(yu + b * dy, 0);
  }
  if (ub == nx) {
    for (int a = nx - 1, b = 0; b < ny; b++)
...
int heat(void) {
  ...
  /* Memory Allocation */
  double **old = (double **)malloc(nx * sizeof(double *));
  double **neww = (double **)malloc(nx * sizeof(double *));

  int l = divide(0, nx, neww, old, ALLC, 0);
  /*
...
int usage(void) {
  ...
  /*
     ...
     fprintf(stderr, "   -xo #    upper x coordinate
     default: 1.570796326794896558\n"); fprintf(stderr, "   -yu #    lower y
     coordinate default: 0.0\n"); fprintf(stderr, "   -yo #    upper y
     coordinate default: 1.570796326794896558\n"); fprintf(stderr, "   -tu #
     start time         default: 0.0\n"); fprintf(stderr, "   -to #    end time
     default: 0.0000001\n");
     */
  return 1;
}
...
void read_heatparams(char *filefn) {
  ...
  int l = fscanf(f, "%d %d %d %lf %lf %lf %lf %lf %lf", &nx, &ny, &nt, &xu, &xo,
                 &yu, &yo, &tu, &to);
  if (l != 9)
