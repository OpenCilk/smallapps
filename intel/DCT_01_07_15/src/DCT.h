...
#pragma pack(push, 1)
typedef struct {
  char filetype[2]; // Indentifies it as a .bmp file. Has filetype[0]='B' and
                    // filetype[1]='M'
  unsigned int filesize;
  short reserved1;
  short reserved2;
  unsigned int dataoffset; // offset in bytes to starting address where the
                           // bitmap data starts
} file_header;

// This is the data structure for Windows 3.x Full header which includes the
// file header too
typedef struct {
  ...
  short planes;
  short bitsperpixel; // This field indentifies if the image is in RGB format
                      // (value 24) or in RGBA format (value 32)
  unsigned int
      compression; // This parameter mentions the type of compression involved.
                   // In our case we are not concerned about the compression but
                   // rather working with the plain image without compression
...
#pragma pack(pop)
// void process_image(rgb *,rgb *,int);