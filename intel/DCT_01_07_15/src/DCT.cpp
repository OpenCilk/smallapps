  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      if (i == 0)
        ...
        x.ptr[(i * size) + j] =
            sqrt((float)2 / size) *
            cosf(((((float)2 * temp[j]) + 1) * i * 3.14f) / (2 * size));
    }
  }
  return;
}

ALIGN void process_image_serial(rgb *indataset, rgb *outdataset,
                                int startindex) {
  int size = 8;
  int size_of_array = size * size;
  matrix_serial dct(size), dctinv(size), interim(size), interim1(size),
      product(size), redinput(size), blueinput(size), greeninput(size),
      quant(size);
  // Quantization matrix which does 50%, 90% and 10% quantization
  float quant50[64] = {16.f, 11.f, 10.f, 16.f, 24.f,  40.f,  51.f,  61.f,
  ...
  float quant90[64] = {3,  2,  2,  3,  5,  8,  10, 12, 2,  2,  3,  4,  5,
                       12, 12, 11, 3,  3,  3,  5,  8,  11, 14, 11, 3,  3,
                       4,  6,  10, 17, 16, 12, 4,  4,  7,  11, 14, 22, 21,
                       15, 5,  7,  11, 13, 16, 12, 23, 18, 10, 13, 16, 17,
                       21, 24, 24, 21, 14, 18, 19, 20, 22, 20, 20, 20};
  float quant10[64] = {80,  60,  50,  80,  120, 200, 255, 255, 55,  60,  70,
                       95,  130, 255, 255, 255, 70,  65,  80,  120, 200, 255,
                       255, 255, 70,  85,  110, 145, 255, 255, 255, 255, 90,
                       110, 185, 255, 255, 255, 255, 255, 120, 175, 255, 255,
                       255, 255, 255, 255, 245, 255, 255, 255, 255, 255, 255,
...
int read_process_write(char *input, char *output, int choice) {
  ...
  // retrieved from input image file
