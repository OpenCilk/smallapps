...
// #endif
    ...
    resized_indataset = (rgb *)aligned_alloc(ALIGNMENT, round_up(sizeof(rgb)*resized_width*resized_height, ALIGNMENT));
	...
	resized_outdataset = (rgb *)aligned_alloc(ALIGNMENT, round_up(sizeof(rgb)*resized_width*resized_height, ALIGNMENT));
...
/*ALIGN*/ int read_process_write(char* input, char *output, int choice) {
	...
	indata = (rgb *)aligned_alloc(ALIGNMENT, round_up(sizeof(rgb)*(hp->width * hp->height), ALIGNMENT));
    if(indata==NULL){
	...
	outdata = (rgb *)aligned_alloc(ALIGNMENT, round_up(sizeof(rgb)*(size_of_image), ALIGNMENT));
    if(outdata==NULL){
        cout<<"Unable to allocate the memory for bitmap date\n";
        return 0;
    }
