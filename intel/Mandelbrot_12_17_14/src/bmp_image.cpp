...
// To learn more how this is laid out, check
...
BMPImage::BMPImage() { clear(); }

BMPImage::BMPImage(unsigned int image_width, unsigned int image_height,
                   unsigned char image_bpp) {
  if (image_bpp != 8 && image_bpp != 24 && image_bpp != 32) {
    throw io::exception(
        "Unsupported number of bits per pixel while creating a BMPImage!");
  }
  clear();
...
BMPImage::BMPImage(const std::string &filename,
                   const std::string &failover_file) {
  load(filename, failover_file);
}

BMPImage::~BMPImage() { clear(); }

void BMPImage::clear() { ImageBase<unsigned char>::clear(); }

unsigned int BMPImage::channels() const { return bpp() / 8; }

/// If @p filename cannot be opened, this routine attempts to load @p
/// failover_file instead. Typically @p failover_file is copied to the directory
/// with the executable after it is built.
bool BMPImage::load(const std::string &filename,
  ...
  if (!failover_file.empty() && !file.is_open()) {
    ...
    if (file.is_open()) {
      ...
      std::cout << "--> Use the default '" << failover_file
                << "' instead? [y] ";
  ...
  if (!file.is_open()) {
    ...
    if (filename.find("..") == 0) {
      message << std::endl
              << "--> Please check the relative path from the current "
                 "directory to the specified image file.";
    }
  ...
  const unsigned int row_size =
      static_cast<unsigned int>(ceil(double(m_bpp * m_width) / 32.0)) * 4;
    ...
    for (unsigned int y = 0; y < m_height; ++y) {
      for (unsigned int x = 0; x < m_width; ++x) {
        for (unsigned int c = 0; c < channels(); ++c) {
          m_image[c + x * channels() + y * m_width * channels()] =
              static_cast<unsigned char>(
                  temp[c + x * channels() + y * row_size]);
        }
      }
    }
    delete[] temp;
        ...
        for (unsigned int x = 0; x < m_width; ++x) {
          for (unsigned int k = 0; k < chan; ++k) {
            unsigned char v = static_cast<unsigned char>(
                m_image[k + x * chan + y * m_width * chan]);
            rowsum[k] += v;
            colsum[x][k] += v;
          }
        }
...
bool BMPImage::save(const std::string &filename) const {
  ...
  file.write(reinterpret_cast<const char *>(&m_file_header),
  ...
  file.write(reinterpret_cast<const char *>(&m_info_header),
  ...
  if (m_bpp <= 8) {
    ...
    file.write(reinterpret_cast<const char *>(m_color_table),
               sizeof(m_color_table));
    if (!file.good()) {
      file.close();
  ...
  if (m_info_header.sizeimage == m_image_size) {
    ...
    for (unsigned int y = 0; y < m_height; ++y) {
      for (unsigned int x = 0; x < m_width; ++x) {
        for (unsigned int c = 0; c < channels(); ++c) {
          temp[c + x * channels() + y * row_size] = static_cast<char>(
              m_image[c + x * channels() + y * m_width * channels()]);
        }
      }
    }
