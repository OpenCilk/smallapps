...
public:
  ImageBase()
      : m_image(0), m_width(0), m_height(0), m_bpp(0), m_image_size(0) {}
  ...
  inline int size() const { return m_image_size; }

  inline T *image() const { return m_image; }

  template <typename I> void from_gray(const I *input) {
  ...
  template <typename I> void from_rgb(I *input) {
    for (unsigned int i = 0; i < m_width * m_height; ++i) {
      switch (channels()) {
      case 1: {
        int idx = 3 * i;
        m_image[i] = static_cast<unsigned char>((float(input[idx]) +
                                                 float(input[idx + 1]) +
                                                 float(input[idx + 2])) /
                                                3.0f);
        break;
  ...
  template <typename O> void to_gray(O *output) {
    for (unsigned int i = 0; i < m_width * m_height; ++i) {
      switch (channels()) {
      case 1: {
      ...
      case 3: {
        int idx = 3 * i;
        output[i] =
            static_cast<O>((float(m_image[idx]) + float(m_image[idx + 1]) +
                            float(m_image[idx + 2])) /
                           3.0f);
        break;
      }
  ...
  template <typename O> void to_rgb(O *output) {
    for (unsigned int i = 0; i < m_width * m_height; ++i) {
      int idx = 3 * i;
      switch (channels()) {
      case 1:
  ...
  /// Returns @p output with the conversion of m_image to the specified number
  /// of @p channels. Assumes that @p channels has a value from 1 to 4, but
  /// converts only for 1 and 3 channels.
  template <typename O> void to_channels(O *output, unsigned int num_channels) {
